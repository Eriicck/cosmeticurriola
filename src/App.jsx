...
function ProtectedAdmin({ children }) {
  ...
  if (status === 'loading') return (
    <div className="min-h-screen bg-[#1a1209] flex items-center justify-center">
      <svg className="animate-spin w-6 h-6 text-[#c9a96e]" viewBox="0 0 24 24" fill="none">
...
function PageTransition({ children }) {
  ...
  useEffect(() => {
    ...
    setShow(false);
    const t = setTimeout(() => { setCurrent(children); setShow(true); prev.current = location.pathname; }, 230);
    return () => clearTimeout(t);
  }, [location.pathname]); // eslint-disable-line

  return (
    <div style={{ opacity: show ? 1 : 0, transition: 'opacity 230ms ease' }}>
      {current}
    </div>
  );
}
...
export default function App() {
  ...
  const cartProps = { cart, cartCount, onAddToCart: addToCart, onOpenCart: () => setIsCartOpen(true) };
