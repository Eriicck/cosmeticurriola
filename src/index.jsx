...
function ReelCard({ reel }) {
  ...
  return (
    <div
      className="relative flex-shrink-0 w-[190px] md:w-[230px] overflow-hidden cursor-pointer bg-gray-900 shadow-xl rounded-2xl"
...
export default function Index({ cartCount = 0, onOpenCart }) {
  ...
  // ── Leer config del admin (se recarga cada vez que se monta la página) ──────
  ...
  useEffect(() => {
    const onScroll = () => {
      ...
      if (heroImgRef.current) heroImgRef.current.style.transform = `translateY(${sy * 0.32}px)`;
      setScrolled(sy > 60);
  ...
  return (
    <div className="bg-[#faf8f5] overflow-x-hidden" style={{ fontFamily: "'Jost', sans-serif" }}>
      ...
      {/* ══ NAVBAR ══════════════════════════════════════════════════════════ */}
      <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${scrolled ? 'bg-white/95 backdrop-blur-md border-b border-[#e8ddd0]' : 'bg-transparent'}`}>
          ...
          <div className="flex items-center gap-6 flex-1">
            ...
            <nav className="hidden md:flex gap-7">
              {NAV_LINKS.map(l => (
                <button
                  key={l.label}
                  onClick={() => l.path.startsWith('/') ? navigate(l.path) : document.querySelector(l.path)?.scrollIntoView({ behavior: 'smooth' })}
        ...
        <div className={`md:hidden bg-white border-t border-[#e8ddd0] overflow-hidden transition-all duration-500 ${menuOpen ? 'max-h-64' : 'max-h-0'}`}>
          <nav className="flex flex-col px-6 py-4 gap-4">
            {[...NAV_LINKS, { label: 'Tienda', path: '/products' }].map(l => (
              <button
                key={l.label}
                onClick={() => { setMenuOpen(false); navigate(l.path); }}
      ...
      <section className="relative w-full overflow-hidden" style={{ height: '100dvh' }}>
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center text-white text-center px-4">
          ...
          <h1 className="anim-fade-up anim-delay-1 text-6xl md:text-8xl lg:text-9xl font-light leading-none tracking-tight mb-5" style={{ fontFamily: "'Cormorant Garamond', serif" }}>
            {hero.title || 'U.RRIOLA'}
          </h1>
        ...
        <div className={`absolute bottom-10 left-1/2 -translate-x-1/2 transition-all duration-500 ${shopByVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4 pointer-events-none'}`}>
          <button onClick={() => navigate('/products')} className="flex flex-col items-center gap-3 group">
          ...
          <div className="max-w-7xl mx-auto px-4 md:px-8">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3 md:gap-4">
              {visibleCats.map((cat) => (
                ...
                >
                  <img src={cat.img} alt={cat.label} loading="lazy" className="w-full h-full object-cover object-center transition-transform duration-700 group-hover:scale-110" />
          ...
          <div className="max-w-7xl mx-auto px-4 md:px-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {editorial.map((s, i) => (
                ...
                >
                  <img src={s.img} alt={s.label} loading="lazy" className="w-full h-full object-cover object-center transition-transform duration-700 hover:scale-105" />
                  <div className="absolute inset-0 bg-gradient-to-t from-black/70 via-black/5 to-transparent" />
                  <div className="absolute top-4 left-4">
                    <span className="text-[9px] font-bold tracking-widest uppercase text-white/90 bg-white/10 backdrop-blur-sm px-3 py-1 rounded-full border border-white/20">{s.tag}</span>
                  </div>
      ...
      {visibleReels.length > 0 && (
        <section className="py-20 bg-[#0e0b07] overflow-hidden">
          ...
          <div
            ref={reelsRef}
            className="flex gap-4 px-4 md:px-8 overflow-x-auto pb-2 hide-scroll select-none"
            style={{ cursor: 'grab' }}
      ...
      <section className="py-14 bg-[#f7f3ee] border-y border-[#e8ddd0] overflow-hidden">
        ...
        <div className="relative flex overflow-hidden">
          {[0, 1].map(copy => (
            <div
              key={copy}
              className="flex gap-10 items-center whitespace-nowrap flex-shrink-0"
              style={{ animation: `marquee 30s linear infinite`, animationDelay: copy === 1 ? '-15s' : '0s' }}
            >
              {[...BRANDS, ...BRANDS].map((b, i) => (
                <span key={i} className="text-sm font-bold tracking-[0.2em] text-[#4a3a31]/50 uppercase hover:text-[#4a3a31] transition-colors duration-300 cursor-default">
                  {b}
                </span>
              ))}
            </div>
      ...
      <section id="nosotros" className="py-24 bg-[#1a1209] text-white">
        <div className="max-w-7xl mx-auto px-4 md:px-8 grid md:grid-cols-2 gap-16 items-center">
          <div>
            ...
            <button onClick={() => navigate('/products')} className="flex items-center gap-3 text-sm font-medium tracking-widest uppercase text-[#c9a96e] hover:gap-5 transition-all duration-300">
      ...
      {/* ══ CTA FINAL ═══════════════════════════════════════════════════════ */}
      <section
        className="relative py-28 text-center px-4 overflow-hidden"
        style={ctaBgStyle()}
      >
        ...
        {cta.type !== 'color' && (
          <div
            className="absolute inset-0 pointer-events-none"
            style={{ background: `rgba(250,248,245,${(cta.overlayOpacity || 40) / 100})` }}
          />
        )}
        {/* Contenido */}
        <div className="relative z-10">
          ...
          <h2
            ...
            style={{ fontFamily: "'Cormorant Garamond', serif", color: cta.type !== 'color' ? 'white' : '#1a1209' }}
          >
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-4 gap-10">
          ...
          <div>
            ...
            <ul className="space-y-2 text-sm">
              {['Inicio', 'Productos', 'Marcas', 'Nosotros'].map(l => (
          ...
          <div>
            ...
            <ul className="space-y-2 text-sm">
              {general.whatsapp && <li>WhatsApp: +{general.whatsapp}</li>}
