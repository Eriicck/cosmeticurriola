          ...
          <div className="p-6 md:p-8 flex flex-col justify-between">
            <div>
                ...
                {product.inOffer && product.offerPrice ? (
                  <>
                    <p className="text-2xl font-bold text-[#c9a96e]">
                      ${Number(product.offerPrice).toFixed(2)} <span className="text-sm font-normal text-gray-400">USD</span>
                    </p>
                    <p className="text-base text-gray-400 line-through">${Number(product.price).toFixed(2)}</p>
                  </>
                ) : (
                  <p className="text-2xl font-bold text-gray-900">
                    ${Number(product.price).toFixed(2)} <span className="text-sm font-normal text-gray-400">USD</span>
                  </p>
                )}
              </div>
              ...
              <p className="text-sm text-gray-500 mt-4 leading-relaxed">
                {product.description || 'Producto de cuidado de la piel K-Beauty premium. Formulado con ingredientes activos de alta concentración para resultados visibles desde la primera semana de uso.'}
              </p>
              ...
              {product.hasTon && product.tonValue && (
                <div className="mt-3 inline-flex items-center gap-2 bg-purple-50 text-purple-600 text-xs font-medium px-3 py-1 rounded-full">
                  <span>🎨</span> Ton: {product.tonValue}
                </div>
              )}
            </div>
            ...
            <div className="mt-6 space-y-3">
              ...
              <button
                ...
                className={`w-full text-white py-3.5 text-sm font-bold tracking-widest rounded-xl transition-all duration-300 shadow-md flex items-center justify-center gap-2
                  ${product.stock !== false ? 'bg-[#4a3a31] hover:bg-[#382b24] hover:shadow-lg cursor-pointer' : 'bg-gray-200 text-gray-400 cursor-not-allowed'}`}
              >
                <ShoppingCart size={16} strokeWidth={1.5} />
                {product.stock !== false
                  ? `Añadir — $${((product.inOffer && product.offerPrice ? product.offerPrice : product.price) * qty).toFixed(2)} USD`
                  : 'Sin stock'}
              </button>
            </div>
          </div>
...
function ProductCard({ product, onAddToCart, onOpenModal }) {
  ...
  return (
    <div className="group relative flex flex-col cursor-pointer">
      <div
        className="relative overflow-hidden bg-gray-50 rounded-sm"
      ...
      >
        ...
        {/* Badge oferta */}
        {product.inOffer && (
          <div className="absolute top-2 left-2 bg-[#c9a96e] text-white text-[9px] font-bold tracking-widest uppercase px-2 py-0.5 rounded-full">
            Oferta
          </div>
        )}
        {/* Badge sin stock */}
      ...
      <div className="mt-4 flex flex-col gap-0.5 px-1 text-center md:text-left">
        ...
        <h3 className="text-[13px] md:text-sm font-medium text-gray-900 leading-snug line-clamp-2 group-hover:text-gray-600 transition-colors duration-300">
          {product.name}
        </h3>
...
useEffect(() => {
  ...
  return (
    <div className="min-h-screen bg-white font-sans text-gray-900">
      <style>{`@import url('https://fonts.googleapis.com/css2?family=Cormorant+Garamond:ital,wght@0,300;1,300&family=Jost:wght@300;400;500;600&display=swap'); * { font-family: 'Jost', sans-serif; } h1,h2 { font-family: 'Cormorant Garamond', serif; }`}</style>
      ...
      <header className="sticky top-0 z-40 w-full bg-white/95 backdrop-blur-sm border-b border-gray-100">
        <div className="flex items-center justify-between px-4 py-4 md:px-8 max-w-7xl mx-auto">
          ...
          <button onClick={() => navigate('/')} className="font-serif text-2xl md:text-3xl tracking-widest font-semibold" style={{ fontFamily: "'Cormorant Garamond', serif" }}>
            U.RRIOLA
          </button>
          ...
          <div className="flex items-center justify-end gap-4 flex-1">
            <button onClick={onOpenCart} className="relative hover:opacity-70 transition-opacity duration-300">
              ...
              {cartCount > 0 && (
                <span className="absolute -bottom-2 -right-2 flex h-5 w-5 items-center justify-center rounded-full bg-gray-900 text-[10px] font-bold text-white">
                  {cartCount}
                </span>
              )}
            </button>
      ...
      <main className="mx-auto w-full max-w-7xl px-4 md:px-8 py-8 flex-1">
        <div className="mb-8 text-center">
          ...
          <h2 className="text-3xl md:text-4xl font-light tracking-tight" style={{ fontFamily: "'Cormorant Garamond', serif" }}>
            Todos los Productos
          </h2>
        </div>

        <div className="flex items-center justify-between border-t border-gray-100 pt-6 mb-6 gap-3 flex-wrap">
          ...
          <div className="flex items-center gap-3">
            <div className="relative">
              ...
              <input
                ...
                className="pl-8 pr-3 py-2 border border-gray-200 rounded-md text-sm focus:outline-none focus:border-gray-400 transition-colors w-40 md:w-52"
              />
            </div>
            <select
              value={sortBy}
              onChange={(e) => setSortBy(e.target.value)}
              className="border border-gray-200 rounded-md py-2 px-3 text-sm text-gray-700 focus:outline-none focus:border-gray-400 cursor-pointer bg-white"
            >
              {SORT_OPTIONS.map(o => <option key={o}>{o}</option>)}
            </select>
            <button
              onClick={() => setFilterOpen(true)}
              className="flex items-center gap-1.5 text-sm font-medium hover:text-gray-600 transition-colors duration-300 border border-gray-200 rounded-md py-2 px-3"
            >
